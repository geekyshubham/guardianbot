...
full import remains open. **Still not claimed:** RouteLens current-binding
scheduled authenticated-full DAST, RouteLens current-binding full DefectDojo
import, least-privilege DefectDojo automation identity hardening, production
model-backed AI review, seven-day enforcement, cross-week monitoring cadence,
GitHub App `pull_request_review_comment` permission, or full PoC acceptance.
Evidence:
...
least-privilege DefectDojo hardening, or GitHub App review-comment permission.
...
full DefectDojo import, least-privilege DefectDojo hardening, weekly cadence
...
is verified), least-privilege DefectDojo automation identity hardening, live
GitHub App feedback events, live descriptor-first PR review consumption /
live pgvector/ANN, weekly cadence acceptance across multiple UTC weeks,
...
`30734622751/1`). Still required: independently verified Test ID and reimport
timestamp for RouteLens current-binding full-DAST run ID, a tested
failed-import alert, and least-privilege automation identity hardening. Older
baseline reimports are retained as historical evidence only.
